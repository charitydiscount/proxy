...
function convert2PToAffiliateUrl(url, affiliateCode, uniqueId) {
  const baseUrl = 'https://event.2performant.com/events/click?ad_type=quicklink';
...
async function get2PAuthHeaders() {
  ...
  const reqBody = { user: { email: process.env.TWOP_EMAIL, password: process.env.TWOP_PASS } }

  const twoPResponse = await fetch('https://api.2performant.com/users/sign_in', {
    method: 'post',
    headers: reqHeaders,
...
async function get2PPrograms(auth) {
  ...
  for (let page = market.metadata.pagination.current_page; page < pages; page++) {
...
async function get2PProgramsForPage(authData, page, perPage) {
  ...
  return convert.toMarket(respBody);
}
